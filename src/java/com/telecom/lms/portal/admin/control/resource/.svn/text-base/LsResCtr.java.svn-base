...
//import com.telecom.lms.sdk.service.resource.ResourceLibraryService;
import com.telecom.lms.sdk.service.resources.ResBaseService;

@Controller
@RequestMapping("/lsRes")
public class LsResCtr {
...
//查询所有历史资源（分页查询）
   @RequestMapping(value="lsResList.html",method=RequestMethod.GET)
   public ModelAndView getCaseDocList(HttpServletRequest req,HttpServletResponse res,ModelMap model){

	    return new ModelAndView("resource/lsResList", model);
  }
   
 //批量移除历史资源
	@RequestMapping(value="deleteLsRes.html",method=RequestMethod.POST)
	@ResponseBody
	public Return deleteLsItems(HttpServletRequest req,HttpServletResponse res,@RequestParam("groupTypeId") String items){
		
		Return re = resBaseService.deleteHistorys(items);
		if(re != null){}
		return re;
	}
	
	//批量删除历史资源
	@RequestMapping(value="deleteAllLsRes.html",method=RequestMethod.POST)
	@ResponseBody
	public Return deleteAllLsItems(HttpServletRequest req,HttpServletResponse res,@RequestParam("groupTypeId") String items){
