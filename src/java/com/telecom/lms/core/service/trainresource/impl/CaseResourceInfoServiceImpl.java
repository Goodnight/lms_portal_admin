...
@Service
@Transactional
public class CaseResourceInfoServiceImpl implements CaseResourceInfoService {

	@Resource(name = "lmsJdbcTemplate")
	private JdbcTemplate jdbcTemplate;

	@Override
	public List<CaseBo> find(CaseResourceInfoSearchForm form) {
		...
		return jdbcTemplate.query(sql.toString(), list.toArray(), new CaseResourceRowMapper());
	}

	class CaseResourceRowMapper implements RowMapper<CaseBo> {

		@Override
		public CaseBo mapRow(ResultSet rs, int rowNumber) throws SQLException {

			CaseBo caseBo = new CaseBo();
			caseBo.setIsDownload(rs.getInt("isDownload"));
	...
	@Override
	public Integer count(CaseResourceInfoSearchForm form) {
	...
	private ExecuteInfo executeSql(CaseResourceInfoSearchForm form) {
		...
		sql.append(" and rb.history = 0 and rb.isDel = 0 and rd.docType = 1 ");
