...
<table class="datatable" width="100%">
	<thead>
		<tr>
			...
			<th id="asd" class="sorting">分享</th>
			<th id="asd" class="sorting">评论</th>
			<th class="sorting">预览</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${libraryList.data }" var="sc" varStatus="st">
			<tr class='<c:out value="${st.index%2==0?'grey':'' }"/>'>
				...
				<td><c:if test="${sc.type ==1 }">
						<a target="_blank"
							href="${basepath }/courseware/coursewareDetails.html?cId=&resId=${sc.rbId }&sn=${sc.sn }"
							title="${sc.sn }"> ${sc.name }</a>
					</c:if> <c:if test="${sc.type ==2}">
						<a target="_blank"
							href="${basepath }/doc/docDetails.html?dId=&rpId=${sc.rbId}&sn=${sc.sn }"
							title="${sc.sn }">${sc.name}</a>
					</c:if> <c:if test="${sc.type ==3}">
						<a
							href="${basepath }/case/caseDocDetails.html?dId=&rpId=${sc.rbId}&sn=${sc.sn }"
				...
				<td><c:if test="${sc.type ==1}">
						<c:forEach items="${sc.cw.data }" var="d">
							...
							<c:if
								...
								<a title="${d.outCode}"
									id="${basepath }/ctu-resource-agent?player=external&resource_id=${d.outCode}&user=${sessionScope.user.uid}&index=${sc.cw.url}&is_trace=false"
									href="javascript:;" onclick="clickshpw(this)">预览</a>
							</c:if>
						</c:forEach>
...
<c:set var="pager" value="${libraryList.page }" scope="request"></c:set>
