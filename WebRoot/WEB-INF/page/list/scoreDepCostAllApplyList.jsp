...
<table class="datatable" width="100%">
                	<thead>
                       ...
                       <c:forEach items="${scoreDepCostList.data }" var="sc" varStatus="st">   
	                        	...
	                        	<td><c:if test="${sessionScope.defaultOrg.name != sc.dep.name}"><c:if test="${sc.stauts == 0 }"><input name="groupTypeId" type="checkbox" value="${sc.sdcId }" id="${sc.score }" class="checkbox cls_checkbox cls_chooseitem"/></c:if></c:if></td>
	                            <td>${sc.begin_time }</td>
	                            <td>${sc.dep.name }</td>
	                            <td>${sc.projectName }</td>
	                            <td>${sc.score }</td>
	                            <td>${sc.costWay.name }</td>
	                            <td><c:if test="${sessionScope.defaultOrg.name != sc.dep.name}">
	                            		<a href="${basepath }/rewardScoreDepCost/scoreDepCostById.html?sdcId=${sc.sdcId}&depId=${sc.dep.orgId}">查看</a>
	                           		 </c:if>
	                            	<c:if test="${sessionScope.defaultOrg.name == sc.dep.name}">
	                            		查看
	                           		 </c:if>
	                            </td>
