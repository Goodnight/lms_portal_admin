...
<table class="datatable" width="100%">
  ...
  <tbody>
  	<c:forEach var="s" items="${list.data }" varStatus="st">
	      ...
	      <td class="pl10">
	      	<c:choose>
	      		<c:when test="${s.status==1 }">
			      	<input id="${s.sId }" delete="1"  name="id" type="checkbox" value="${s.sId }" partinnum="${s.participantsNum}" class="checkbox choose_item_est"/>
	      		</c:when>
	      		<c:otherwise>
			      	<input id="${s.sId }" delete="0" name="id" type="checkbox" value="${s.sId }" partinnum="${s.participantsNum}" class="checkbox choose_item_est"/>
	      		</c:otherwise>
	      	</c:choose>
	      </td>
	      <td>
              ...
              <!-- 1.已发布： 只能修改人员 -->
              <c:if test="${s.status==2 }"><a href="new.html?type=4&sid=${s.sId }&symbol=1">${s.topic }</a></c:if>
              <!-- 2.未发布 && 有人参与： 只能修改开始结束时间 和 参与人员 -->
              <c:if test="${s.status==1 && s.participantsNum!=0 }"><a href="new.html?type=4&sid=${s.sId }&symbol=2">${s.topic }</a></c:if>
              <!-- 3.未发布 && 无人参与： 皆可修改 -->
              <c:if test="${s.status==1 && s.participantsNum==0 }"><a href="new.html?type=4&sid=${s.sId }&symbol=3">${s.topic }</a></c:if>
          </td>
	      <td>${s.startDt }</td>
	      <td>${s.endDt }</td>
	      ...
	      <td><a href="${basepath }/survey/viewParticipate.html?sId=${s.sId }&flag=comprehensive">${s.participantsNum==null?0:s.participantsNum }/${s.userNum==null?0:s.userNum }</a></td>
	      <td><a href="${basepath}/survey/result/report.html?report=comprehensive&surveyId=${s.sId }" target="_blank">结果明细</a></td>
