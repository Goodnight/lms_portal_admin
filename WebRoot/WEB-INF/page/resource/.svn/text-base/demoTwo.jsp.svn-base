	...
	window.onload=function(){
		...
		swf = ctu_swf_uploader(settings,path,getUploadParams,function(file, fileData) {
			...
			for ( var i = 0; i < files.length; i++) {
				...
				form_saveDoc.outCode.value=files[i].id;
		...
		var settings1 = {
		file_size_limit : "400MB",
		file_types : "*.xls;*.xlsx;*.doc;*.docx;*.ppt;*.pptx;*.pdf;*.wps",
		file_post_name : "uploadFile",
		...
		swf1 = ctu_swf_uploader(settings1,path,getUploadParams1,function(file, fileData) {
			...
			for ( var i = 0; i < files.length; i++) {
				...
				var newRow = root.insertRow(-1);
				var newCell0 = newRow.insertCell(-1);
				form_saveDoc.uplodSrc.value=files[i].id;
...
<body>
	...
	<tr>
	<td>上传封面<em>(规定尺寸不超过:65*29,格式:Jpg,Png)</em><c:if test="${doc.res.pic.outCode != null && doc.res.pic.outCode != ''}"><img src="${donwlodResourseURL }/ctu-resource-agent?uri_type=download&user=${sessionScope.user.uid}&resource_id=${doc.res.pic.outCode}" width="114" height="84" /></c:if></td>
	<td colspan="3">
...
<tr id="upLodDoc">
		<td><em>*</em>上传文档附件<em>(*.xls;*.xlsx;*.doc;*.docx;*.ppt;*.pptx;*.pdf;*.wps)</em><br>
		<c:if test="${b != null && b != ''}">
		<a id="" href="${donwlodResourseURL }/ctu-resource-agent?uri_type=download&user=${sessionScope.user.uid}&resource_id=${b}&player=document" class="red">下载:${docResourceInfo.meta.full_name }</a></c:if></td>
		<td colspan="3">
