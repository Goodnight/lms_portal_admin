        	...
        	<div class="ngreyborder changeblue">
                ...
                <div class="courselist">
                    <ul class="png_bg">
                     ...
                     <!-- 文档列表 -->
                    <div id="onlineDocByCaseDocList_list"></div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$(".cls_ok").click(function(){
			...
			if(ids.length>0){
				var param = "method=add&res=doc&rdId="+rdId;
	...
	function page(i){
		var url = basepath + "/list/resource/onlineDocByCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random();
		$("#onlineDocByCaseDocList_list").load(encodeURI(url),function(){
			//禁用已经选择的选项
...
function selectDocBottonClick(i){
	...
	query+="&isChildOrg="+isChildOrg;
		var url = basepath + "/list/resource/onlineDocByCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random() + "&docName=" + docName + "&docSn=" + docSn + "&docJiGou=" + docJiGou;
		$("#onlineDocByCaseDocList_list").load(encodeURI(url+query),function(){
			//禁用已经选择的选项
			var cbList = $("input:checkbox[name=rdId]");
			var ctList = $("input[name=choosedDoc]");
			if(cbList.length>0 && ctList.length>0){
