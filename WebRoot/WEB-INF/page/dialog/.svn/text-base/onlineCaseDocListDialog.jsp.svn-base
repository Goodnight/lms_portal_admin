        	...
        	<div class="ngreyborder changeblue">
                ...
                <div class="courselist">
                    <ul class="png_bg">
                     ...
                     <!-- 案例列表 -->
                    <div id="onlineCaseDocList_list"></div>
                   
                     
                     </div>
            	</div>
                 <div class="choosedCaseDoc">
                 	<ul class="png_bg">
...
<script type="text/javascript">
	...
	$(".cls_ok").click(function(){
			...
			if(ids.length>0){
				var param = "method=add&res=caseDoc&cId="+cId;
	...
	function page(i){
		var url = basepath + "/list/resource/onlineCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random();
		$("#onlineCaseDocList_list").load(encodeURI(url),function(){
			//禁用已经选择的选项
		...
		function pxu(i){
		if(document.getElementById("ll").onclick)
		{
		var url = basepath + "/list/resource/onlineCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random() + "order=views";
		}
		if(document.getElementById("xz").onclick)
		{
		var url = basepath + "/list/resource/onlineCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random() + "order=downloads";
		}
		$("#onlineCaseDocList_list").load(encodeURI(url),function(){
			//禁用已经选择的选项
			var cbList = $("input:checkbox[name=rdId]");
...
function selectCaseDocBottonClick(i){
	...
	var url = basepath + "/list/resource/onlineCaseDocList.html?pagefn=page&page="+i+"&r="+Math.random();
		$("#onlineCaseDocList_list").load(encodeURI(url+query),function(){
			//禁用已经选择的选项
			var cbList = $("input:checkbox[name=rdId]");
			var ctList = $("input[name=choosedCaseDoc]");
			if(cbList.length>0 && ctList.length>0){
