...
<%@include file="/WEB-INF/page/include/taglibs.jsp"%>
        	...
        	<div class="ngreyborder changeblue">
                ...
                <div class="courselist">
                    ...
                    <!-- 课程列表 -->
                    <div id="onlineCoursewareList_list"></div>
...
<script type="text/javascript">
	...
	//提交保存数据
		$(".cls_ok").click(function(){
			...
			if(ids.length>0){
				...
				var param = "method=add&res=courseware&cId="+cId;
	...
	//添加课程
	function add(obj){
		var newTr = "<tr><td><input type='button' class='removebutton' id='"+$(obj).val()+"' name='choosedCourse' onclick='remove(this)'/></td>"
	...
	function page(i){
		var url = basepath + "/list/resource/onlineCoursewareList.html?pagefn=page&page="+i+"&r="+Math.random();
		$("#onlineCoursewareList_list").load(encodeURI(url),function(){
			//禁用已经选择的选项
	...
	var url = basepath + "/list/resource/onlineCoursewareList.html?pagefn=page&page="+i+"&r="+Math.random();
		$("#onlineCoursewareList_list").load(encodeURI(url+query),function(){
			//禁用已经选择的选项
			var cbList = $("input:checkbox[name=cId]");
			var ctList = $("input[name=choosedCourse]");
			if(cbList.length>0 && ctList.length>0){
