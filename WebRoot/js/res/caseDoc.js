...
function searchBottonClick(i) {
	...
	query += "&name=" + name;
	var url = basepath + "/list/toCaseDocList.html?pagefn=page&page=" + i
...
function export_case() {
	...
	loadingDataStart();
	var countUrl = basepath + "/case/exportCount.html?r="+Math.random();
	var listUrl = basepath + "/case/exportList.html?r="+Math.random();
...
function selectBottonClick(i) {
	...
	query += "&isChildKnowledge=" + isChildKno;
	var url = basepath
			+ "/list/toCaseDocList.html?pagefn=selectBottonClick&page=" + i
			+ "&max=" + max + "&r=" + Math.random();
	$("#list_trainclass").load(encodeURI(url + query), function() {
		$(".cls_chooseall").click(function() {
			if ($(this).attr("checked")) {
...
function toDocPic(i) {
	...
	var url = basepath + "/list/toCaseDocPicList.html?pagefn=toDocPic&page="
...
function seachJHClick(i, obj) {
	...
	if (index == 1) {
		...
		var url = basepath
				+ "/list/toCaseDocPicList.html?pagefn=toDocPic&page=" + i
				+ "&max=" + max + "&r=" + Math.random();
		$("#list_docPic").load(encodeURI(url + query), function() {
			$(".cls_chooseall").click(function() {
				if ($(this).attr("checked")) {
	...
	if (param == "0" || param == "" || param == null) {
		...
		$.ajax({
			...
			complete : function(data) {
				...
				nw.location = "tosaveCaseDocDetails.html?knoId=" + knoId;
			}
		});
	}
});
