...
function searchBottonClick(i){
	...
	query += "&name="+name;
	var url = basepath+"/list/toCaseDocList.html?pagefn=page&page="+i+"&max="+max+"&r="+Math.random();
...
function selectBottonClick(i){
	...
	var url = basepath+"/list/toCaseDocList.html?pagefn=selectBottonClick&page="+i+"&max="+max+"&r="+Math.random();
	$("#list_trainclass").load(encodeURI(url+query),function(){
		$(".cls_chooseall").click(function(){
			if($(this).attr("checked")){
				$(".cls_chooseitem").attr("checked",true);
...
function toDocPic(i){
	...
	var url = basepath+"/list/toCaseDocPicList.html?pagefn=toDocPic&page="+i+"&max="+max+"&r="+Math.random();
	...
	if(param == "0" || param == "" || param == null){
		...
		$.ajax({
			...
			complete : function(data){
				...
				nw.location = basepath+"/case/tosaveCaseDocDetails.html?knoId="+knoId;
			}
		});
    }
});
